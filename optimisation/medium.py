...
def find_maximum(function):
    ...
    while True:
        ...
        next_solution = get_best_neighbour(*current_solution)

        if utility(*current_solution) < next_solution[0]:
            current_solution = next_solution[1]
        else:
...
def get_best_neighbour(x, y):

    neighbours = get_neighbours(x, y)
    utility_values = []

    for neighbour in neighbours:
        get_utility = utility(*neighbour)
        utility_values.append((get_utility, neighbour))
